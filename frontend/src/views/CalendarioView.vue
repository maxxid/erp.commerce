  ...
  <div class="p-6 space-y-6">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-5">
      <div class="flex items-center gap-4">
        <label class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Fecha</label>
        <input
          v-model="selectedDate"
          type="date"
          class="border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-600/20 focus:border-brand-600 outline-none transition-all"
        />
        <div class="flex items-center gap-1.5 ml-auto">
          <button
            ...
            :class="activeTab === tab.key ? 'bg-brand-600 text-white shadow-sm' : 'text-gray-500 hover:bg-gray-100'"
          >
            {{ tab.label }}
          </button>
        </div>
    ...
    <div v-show="activeTab === 'todo' || activeTab === 'ventas'" class="bg-white rounded-2xl shadow-sm p-5">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-gray-900">Ventas del día</h3>
            <p class="text-xs text-gray-500">{{ selectedDate }}</p>
          </div>
        </div>
        <div class="text-right">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Total</p>
          <p class="text-xl font-mono-data font-bold text-gray-900">{{ formatCurrency(dailySalesTotal) }}</p>
        </div>
      </div>
      <div class="grid grid-cols-3 gap-4 mb-4">
        <div class="bg-gray-50 rounded-xl p-4 text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Tickets</p>
          <p class="text-lg font-mono-data font-bold text-gray-900">{{ dailySales.length }}</p>
        </div>
        <div class="bg-gray-50 rounded-xl p-4 text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Promedio</p>
          <p class="text-lg font-mono-data font-bold text-gray-900">{{ formatCurrency(dailySalesAvg) }}</p>
        </div>
        <div class="bg-gray-50 rounded-xl p-4 text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Productos vendidos</p>
          <p class="text-lg font-mono-data font-bold text-gray-900">{{ totalItemsSold }}</p>
        </div>
      </div>
      <details class="group">
        ...
        <div class="mt-3 space-y-2">
          <div v-for="sale in dailySales" :key="sale.id" class="flex items-center justify-between px-4 py-2.5 bg-gray-50 rounded-xl text-sm">
            <span class="text-gray-700 font-mono-data">Ticket #{{ String(sale.id).padStart(6, '0') }}</span>
            <span class="text-gray-500">{{ sale.time }}</span>
            <span class="text-gray-500">{{ sale.paymentMethod }}</span>
            <span class="font-mono-data font-medium text-gray-900">{{ formatCurrency(sale.total) }}</span>
          </div>
        </div>
      </details>
    </div>
    ...
    <div v-show="activeTab === 'todo' || activeTab === 'caja'" class="bg-white rounded-2xl shadow-sm p-5">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-gray-900">Movimientos de caja</h3>
            <p class="text-xs text-gray-500">{{ selectedDate }}</p>
          </div>
        </div>
        <div class="text-right">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Balance</p>
          <p class="text-xl font-mono-data font-bold" :class="cashBalance >= 0 ? 'text-green-600' : 'text-red-600'">{{ formatCurrency(cashBalance) }}</p>
        </div>
      </div>
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="bg-green-50 rounded-xl p-4 text-center">
          <p class="text-[10px] uppercase tracking-wider text-green-600 font-semibold">Ingresos</p>
          <p class="text-lg font-mono-data font-bold text-green-700">{{ formatCurrency(cashInTotal) }}</p>
        </div>
      ...
      <details class="group">
        ...
        <div class="mt-3 space-y-2">
          <div v-for="mov in cashMovements" :key="mov.id" class="flex items-center justify-between px-4 py-2.5 bg-gray-50 rounded-xl text-sm">
            <div class="flex items-center gap-2">
              <span :class="mov.type === 'ingreso' ? 'text-green-600' : 'text-red-600'">
                <i :class="mov.type === 'ingreso' ? 'fa-solid fa-circle-arrow-down' : 'fa-solid fa-circle-arrow-up'"></i>
              </span>
              <span class="text-gray-700">{{ mov.description }}</span>
            </div>
            <span class="text-gray-500 text-xs">{{ mov.time }}</span>
            <span class="font-mono-data font-medium" :class="mov.type === 'ingreso' ? 'text-green-600' : 'text-red-600'">{{ mov.type === 'ingreso' ? '+' : '-' }}{{ formatCurrency(mov.amount) }}</span>
          </div>
        </div>
      </details>
    </div>
    ...
    <div v-show="activeTab === 'todo' || activeTab === 'compras'" class="bg-white rounded-2xl shadow-sm p-5">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-gray-900">Compras recibidas</h3>
            <p class="text-xs text-gray-500">{{ selectedDate }}</p>
          </div>
        </div>
        <div class="text-right">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Total compras</p>
          <p class="text-xl font-mono-data font-bold text-gray-900">{{ formatCurrency(purchasesTotal) }}</p>
        </div>
      </div>
      <div class="grid grid-cols-2 gap-4 mb-4">
        <div class="bg-gray-50 rounded-xl p-4 text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Recepciones</p>
          <p class="text-lg font-mono-data font-bold text-gray-900">{{ purchases.length }}</p>
        </div>
        <div class="bg-gray-50 rounded-xl p-4 text-center">
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold">Ítems recibidos</p>
          <p class="text-lg font-mono-data font-bold text-gray-900">{{ totalItemsPurchased }}</p>
        </div>
      </div>
      <details class="group">
        <summary class="cursor-pointer text-sm font-medium text-brand-600 hover:text-brand-700 transition-colors flex items-center gap-1.5">
        ...
        <div class="mt-3 space-y-2">
          <div v-for="purchase in purchases" :key="purchase.id" class="flex items-center justify-between px-4 py-2.5 bg-gray-50 rounded-xl text-sm">
            <span class="text-gray-700">{{ purchase.supplier }}</span>
            <span class="text-gray-500 text-xs">{{ purchase.time }}</span>
            <span class="text-gray-500">{{ purchase.items }} items</span>
            <span class="font-mono-data font-medium text-gray-900">{{ formatCurrency(purchase.total) }}</span>
          </div>
        </div>
      </details>
    </div>
    ...
    <div v-show="activeTab === 'todo' || activeTab === 'productos'" class="bg-white rounded-2xl shadow-sm p-5">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-gray-900">Productos nuevos / modificados</h3>
            <p class="text-xs text-gray-500">{{ selectedDate }}</p>
          </div>
        </div>
        <div class="flex items-center gap-3">
          <span class="px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">{{ newProducts.length }} nuevos</span>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold mb-2">Productos nuevos</p>
          <div class="space-y-1.5">
            <div v-for="prod in newProducts" :key="prod.id" class="flex items-center justify-between px-3 py-2 bg-green-50 rounded-lg text-sm">
              <span class="text-gray-800">{{ prod.name }}</span>
              <span class="font-mono-data text-gray-600 text-xs">{{ prod.code }}</span>
            </div>
          </div>
        </div>
        <div>
          <p class="text-[10px] uppercase tracking-wider text-gray-500 font-semibold mb-2">Productos modificados</p>
          <div class="space-y-1.5">
            <div v-for="prod in modifiedProducts" :key="prod.id" class="flex items-center justify-between px-3 py-2 bg-amber-50 rounded-lg text-sm">
              <span class="text-gray-800">{{ prod.name }}</span>
              <span class="text-gray-500 text-xs">{{ prod.change }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-show="activeTab === 'todo' || activeTab === 'clientes'" class="bg-white rounded-2xl shadow-sm p-5">
      <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <h3 class="font-semibold text-gray-900">Nuevos clientes</h3>
            <p class="text-xs text-gray-500">{{ selectedDate }}</p>
          </div>
        </div>
      ...
      <div class="space-y-1.5" v-if="newClients.length > 0">
        <div v-for="client in newClients" :key="client.id" class="flex items-center justify-between px-4 py-2.5 bg-cyan-50 rounded-xl text-sm">
          ...
          <span class="text-gray-500">{{ client.docType }} {{ client.docNumber }}</span>
          <span class="text-gray-400 text-xs">{{ client.time }}</span>
        </div>
      </div>
